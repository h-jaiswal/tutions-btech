...
// Below is a node and start pointer
struct node
{
    int data;
    struct node *next;
...
struct node* create_ll(struct node *start)
{
    ...
    int num;
    printf("\n Enter -1 to end"); // If the user wants to stop creating new nodes, he can enter -1
    printf("\n Enter the data : ");
    scanf("%d", &num);
    ...
    while(num!=-1)
    {
        ...
        new_node -> data = num;
        if(start==NULL){
            new_node -> next = NULL;    // Since, if start==NULL, means this is the first AND ONLY NODE as of now, therefore next=NULL
            start = new_node;
        }
        else
        {
...
int numNodes(struct node *start)
{
    ...
    while(ptr != NULL)
    {
        count++;       // Read, Print, Manipulation of linked list Node data
        ptr = ptr -> next;
    }
    return count;
}
